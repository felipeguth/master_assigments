...
int main()
{
 ...
 int vetWinnersNeverLost[numElements/2]; //foram vencedores e nunca perderam em comparações
 int vetLosersNeverWin[numElements/2]; // perdedores que nunca venceram em comparações
 ...
 srand(time(NULL));
  for (i=0; i<numElements; i++)
      {
           /* para gerar números aleatórios de 0 a 10000 */
          // printf("%d \n", rand() % 1000);
 ...
 do{
    ...
    i++;
    j--;
    nCompar++; //incrementa total de comparações 
 }while (cont >= 2); 
 ...
 do{  //elimina um valor por vez, passa n/2 - 1 vezes uma vez que vet tem n/2 elementos
  //  printf("\n first = %d, last = %d", vetWinnersNeverLost[i], vetWinnersNeverLost[j]);
    ...
    if(vetWinnersNeverLost[i] > vetWinnersNeverLost[j])
    {
     ...
     vetWinnersNeverLost[j] = 0;    //simula liberar espaço
    ...
    else 
    {
     ...
     vetWinnersNeverLost[i] = 0;       //simula liberar espaço
 ...
 do{  //elimina um valor por vez, passa n/2 - 1 vezes uma vez que vet tem n/2 elementos
  ...
  //printf("\n firsteD = %d, lasteD = %d", vetLosersNeverWin[i], vetLosersNeverWin[j]);
    ...
    {      
     ...
     vetLosersNeverWin[j] = 0; //simula liberar espaço
     j--;         //compara i novamente com próximo j - decrescente     
    }
    else //el i maior
    {      
     ...
     vetLosersNeverWin[i] = 0;     //simula liberar espaço
     i++;       //compara j novamente com próximo i  - crescente    
    }       
    cont = cont--; 
    contWL++;
